...
\section{Cấu trúc chỉ mục nâng cao: Forward Index và Inverted Index}
Nhằm cải tiến tốc độ và tối ưu cấu trúc dữ liệu lưu trữ trong quá trình truy vấn đặc trưng ảnh, hệ thống tích hợp hai cấu trúc chỉ mục: \textbf{Forward Index} (Chỉ mục xuôi) và \textbf{Inverted Index} (Chỉ mục ngược).

\subsection{Forward Index (Chỉ mục xuôi)}
Forward Index lưu trữ ánh xạ xuôi từ một ảnh (thông qua mã định danh \texttt{imageID}) đến danh sách các vector đặc trưng của ảnh đó. Cấu trúc lưu trữ sử dụng bảng băm (lớp \texttt{std::unordered\_map} trong C++):
\begin{equation}
\text{ForwardIndex}: \texttt{imageID} \to \{ \text{Phương pháp}_i \to \text{Vector đặc trưng}_i \}
\end{equation}
Trong các hệ thống CBIR thông thường, CSDL được biểu diễn như một mảng tuyến tính. Khi cần đối chiếu khoảng cách Chi-Squared cho các đặc trưng như Color Histogram hay LBP, việc duyệt qua từng cấu trúc bản ghi và tìm kiếm trong bản đồ đặc trưng có độ phức tạp trung bình khá lớn. Forward Index giúp tra cứu trực tiếp vector đặc trưng của bất kỳ ảnh nào theo ID với độ phức tạp $O(1)$.

\subsection{Inverted Index (Chỉ mục ngược)}
Đối với các phương pháp trích xuất đặc trưng dạng túi từ vựng (Bag of Visual Words) như SIFT và ORB, biểu diễn đặc trưng của ảnh là một histogram thưa (sparse histogram) trên một từ điển từ vựng (vocabulary) gồm $V$ từ ($V=500$). 

Inverted Index tận dụng tính chất thưa này bằng cách lập chỉ mục ngược từ một từ vựng thị giác (\texttt{visual wordID}) sang danh sách các ảnh có chứa từ vựng đó kèm theo tần suất xuất hiện (\texttt{Posting List}):
\begin{equation}
\text{InvertedIndex}: \text{wordID} \to [ (\text{imageID}_1, tf_1), (\text{imageID}_2, tf_2), \dots ]
\end{equation}

Khi thực hiện truy vấn với một ảnh query có histogram biểu diễn là $\mathbf{q}$:
\begin{enumerate}
    \item Hệ thống chỉ quan tâm đến các từ vựng $t$ có trọng số $q_t > 0$ trong ảnh query.
    \item Với mỗi từ vựng $t$ đó, hệ thống truy xuất Posting List tương ứng để lấy danh sách các ảnh ứng viên chứa từ $t$.
    \item Điểm tương đồng giữa ảnh query và ảnh ứng viên $d$ được tích lũy theo mô hình không gian vector kết hợp trọng số TF-IDF:
    \begin{equation}
    \operatorname{Score}(\mathbf{q}, d) = \sum_{t \in \mathbf{q} \cap d} tf_{t,\mathbf{q}} \times idf_t \times tf_{t,d}
    \end{equation}
    Trong đó:
    \begin{itemize}
        \item $tf_{t,d}$: Tần suất xuất hiện của từ $t$ trong tài liệu $d$ (giá trị bin thứ $t$ đã lượng hóa).
        \item $idf_t = \ln(N / df_t)$: Trọng số nghịch đảo tần suất tài liệu của từ $t$, với $N$ là tổng số ảnh trong hệ thống và $df_t$ là số ảnh chứa từ $t$.
    \end{itemize}
\end{enumerate}
Phương pháp này giúp giảm độ phức tạp tìm kiếm từ việc quét toàn bộ CSDL $O(N)$ xuống chỉ còn duyệt qua tập ảnh ứng viên chứa chung từ vựng thị giác với ảnh query, đồng thời tăng độ chính xác thông qua việc lọc bỏ các từ vựng quá phổ biến (có $idf_t$ nhỏ).
